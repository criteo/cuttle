...
private[timeseries] trait TimeSeriesApp { self: TimeSeriesScheduler =>
  ...
  private[cuttle] override def publicRoutes(workflowJobs: Workload[TimeSeries],
                                            executor: Executor[TimeSeries],
                                            xa: XA): PartialService = {
    val workflow = workflowJobs.asInstanceOf[Workflow]
    ({
  ...
  private[cuttle] override def privateRoutes(workflowJobs: Workload[TimeSeries],
                                             executor: Executor[TimeSeries],
            ...
            _.as[BackfillCreate]
              .fold(
                ...
                backfill => {
                  ...
                  val jobs = workflowJobs.asInstanceOf[Workflow].vertices
    ...
    case req @GET at url"/api/timeseries/force-success?job=$jobId&start=$start&end=$end" => {
      implicit user =>
        (for {
          ...
          job <- Try(workflowJobs.asInstanceOf[Workflow].vertices.find(_.id == jobId).getOrElse(throw new Exception(s"Unknow job $jobId")))
        } yield {
          val requestedInterval = Interval(startDate, endDate)
