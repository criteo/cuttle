...
import com.criteo.cuttle.authentication.{NoAuth, OAuth2Authenticator, OAuth2ClientConfig}
...
private[cuttle] case class App[S <: Scheduling](project: CuttleProject[S], executor: Executor[S], xa: XA) {
  ...
  val authentifiedApi: AuthentifiedService = {
    case GET at "/private" =>
      _ =>
        Ok("PRIVATE")
  }
  ...
  val routes = NoAuth(authentifiedApi)
    .orElse(publicApi)
