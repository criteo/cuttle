...
export class AnnotatedGraph {
  ...
  getEdges({from, to}: $Shape<{from: string[], to: string[]}>): AnnotatedEdge[] {
    const filterSource = typeof(from) === 'undefined' ? (edge => true) : (edge => from.includes(edge.source));
    const filterTarget = typeof(to) === 'undefined' ? (edge => true) : (edge => to.includes(edge.target));
    return this.edges.filter(e => filterSource(e) && filterTarget(e));
  }

  static build(nodeId: string, graphDB: Graph): AnnotatedGraph {
    const mainOriginalNode = graphDB.getNode(nodeId) || {id: nodeId, order: 0, yPosition: 0};
    ...
    const parentNodes: AnnotatedNode[] = parents.map((el, i) => ({...el, kind: nodeKind.parent}));
    const childNodes: AnnotatedNode[] = children.map((el, i) => ({...el, kind: nodeKind.child}));
    const mainNode: AnnotatedNode = {...mainOriginalNode, kind: nodeKind.main};

    const parentToCenterLinks: AnnotatedEdge[] = graphDB.getEdges({from: parentIds, to: [nodeId]}).map(e => ({...e, kind: edgeKind.parentToCenter}));
    const centerToChildLinks: AnnotatedEdge[] = graphDB.getEdges({from: [nodeId], to: childrenIds}).map(e => ({...e, kind: edgeKind.centerToChild}));
    const parentToMissingChildLinks: AnnotatedEdge[] = graphDB.getEdges({from: parentIds}).filter(e => e.target !== nodeId).map(e => ({...e, kind: edgeKind.parentToMissingChild}));
    const missingParentToChildLinks: AnnotatedEdge[] = graphDB.getEdges({to: childrenIds}).filter(e => e.source !== nodeId).map(e => ({...e, kind: edgeKind.missingParentToChild}));
    const childToMissingChildLinks: AnnotatedEdge[] = graphDB.getEdges({from: childrenIds}).map(e => ({...e, kind: edgeKind.childToMissingChild}));
    const missingParentToParentLinks: AnnotatedEdge[] = graphDB.getEdges({to: parentIds}).map(e => ({...e, kind: edgeKind.missingParentToParent}));
