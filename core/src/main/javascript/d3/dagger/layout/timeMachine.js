...
export type TransitionAction = (arg: $Shape<{
  ...
  pathFrom: any, pathTo: any, previousNode: string
}>) => Promise<*>;
...
const computeDijkstraPath = (minimap, start) => {
  ...
  return end => _.compact(path.pathTo(minimap.getElementById(end)).map(n => n.isNode() ? n.id() : null));
};

export const timeMachineGenerator = (lm: LayoutManager, history: string[], currentNode: string, minimap: any) => {
  ...
  const timeMachine = {
    node: currentNode,
    history: historyAccess, next, back
  };
  return (init: TransitionAction = () => Promise.resolve({})) =>
    init({
      layout: [lm.layout[currentNode], lm.layout[currentNode]],
      annotatedGraph: [lm.annotatedGraph[currentNode], lm.annotatedGraph[currentNode]],
      node: [currentNode, currentNode],
      history: historyAccess, back, next,
      allLayouts: lm.layout,
      previousNode: currentNode
    }).then(value => ({value, timeMachine}))
}

const nextBuilder = (lm: LayoutManager, currentNode: string, history: string[], minimap: any) => {
  ...
  return (nodeId: string, action: TransitionAction): Promise<NextReturn> => {
    ...
    const nextTimeMachine = {
      node: nodeId,
      history: nextHistoryAccess, next, back
    };
    return action({
...
const backBuilder = (lm: LayoutManager, currentNode: string, history: string[], minimap: any) => {
  const [previousNode, ...previousHistory] = history;
  const previousHistoryAccess = historyBuilder(previousHistory);
  const unresolvedPath = computeDijkstraPath(minimap, currentNode);
  return (relPos: number = 1, action: TransitionAction): Promise<NextReturn> => {
    ...
    const previousTimeMachine = {
      node: previousNode,
      history: previousHistoryAccess, next, back
    };
    return action({
      layout: finalPath.map(n => lm.layout[n]),
      node: finalPath,
