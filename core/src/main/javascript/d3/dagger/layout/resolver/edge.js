...
export const resolveEdgesForParentNode = (
  ...
  scale: any
) => graph.getEdges({from: [annotatedNode.id]})
...
export const resolveEdgesForChildNode = (
  ...
  graph: AnnotatedGraph,
  scale: any
) => graph.getEdges({to: [annotatedNode.id]})
...
export const resolveBorderEdges = (
  ...
  const allEdges = _.uniqBy(_.concat(...graphs[mainId].nodes.map(n => graphs[n.id].edges)), 'id');
  ...
  return allEdges.reduce((acc, current) => {
    ...
    return {
      ...acc, [current.id]: {
        ...
        y1: sourceNodeStart.y - sourceNodeStart.height/2 + sourceScale(targetNodeStart.y),
        x2: targetNodeStart.x + targetNodeStart.width/2,
        y2: targetNodeStart.y - targetNodeStart.height/2 + targetScale(sourceNodeStart.y),
        ...current,
        kind
      }
    };
