...
import {AnnotatedGraph, AnnotatedNode, nodeKind} from '../symbolic/annotatedGraph';
...
const scaleBuilder = (fullHeight, nodeHeight) => d3.scaleLinear().domain([0, fullHeight]).range([0, nodeHeight]);

export const resolveFixedNodes = (parents: AnnotatedNode[],
  children: AnnotatedNode[],
  graph: AnnotatedGraph,
  dimensions: GraphDimensions
): ResolvedGraphLayout => {
  const {width: parentNodeWidth, height: parentNodeHeight} = dimensions.nodeSize(parents.length);
  const {width: childNodeWidth, height: childNodeHeight} = dimensions.nodeSize(children.length);
  const mainNode = graph.findNodesByTag(nodeKind.main);
  const main = mainNode.length > 0 ? mainNode[0] : {id: "unknown", order: -1, yPosition: -1, kind: nodeKind.main};
  ...
  const parentPositions = parents.reduce((acc, current, i) => {
    ...
    edges = {...edges, ...resolveEdgesForParentNode(newNodePosition, current, mainPosition, graph, scale)};
  ...
  const childrenPositions = children.reduce((acc, current, i) => {
    ...
    edges = {...edges, ...resolveEdgesForChildNode(newNodePosition, current, mainPosition, graph, scale)};
    return {...acc, [current.id]: newNodePosition};
  }, {});

  return {nodes: {...parentPositions, ...childrenPositions, [main.id]: mainPosition}, edges};
}
