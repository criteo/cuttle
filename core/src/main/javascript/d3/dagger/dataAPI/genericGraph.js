...
export class Graph {
  ...
  getParents(nodeId: string): Node[] {
    return filter(
      this.nodes,
      n => filter(this.edges, { source: n.id, target: nodeId }).length > 0
    );
  }
  ...
  getChildren(nodeId: string): Node[] {
    return filter(
      this.nodes,
      n => filter(this.edges, { source: nodeId, target: n.id }).length > 0
    );
  }

  getEdges({ from, to }: $Shape<{ from: string[], to: string[] }>): Edge[] {
    const filterSource = isUndefined(from)
      ? constant(true)
      : edge => includes(from, edge.source);
    const filterTarget = isUndefined(to)
      ? constant(true)
      : edge => includes(to, edge.target);
