...
import DagIcon from "react-icons/lib/md/zoom-out-map";
...
type Option = {
  value: string,
  label: string,
  kind?: "job" | "tag" | "parents" | "children" | "dag",
  job?: string,
  others?: string[]
};
    ...
    this.options = _.flatMap(workflow.tags, ({ name }) => {
      ...
      .concat(
        _.flatMap(_.sortBy(workflow.jobs, job => job.name), job => {
          ...
          let parents = workflow.getJobParents(job.id);
          let children = workflow.getJobChildren(job.id);
          if (parents.length > 0) {
            result.push({
      ...
      <span>
        ...
        {others && others.length > 0 && (
          <em className={classes.more}>
            {`${kind != "tag" && kind != "dag" ? "+" : ""}${others.length} job${
              others.length > 1 ? "s" : ""
            }`}
          </em>
        )}
